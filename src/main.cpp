...
unsigned int nTargetSpacing = 1 * 18; // 18 seconds
...
const string strMessageMagic = "Rebel Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 <= 13901)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 19476)
    {
      nSubsidy = 30 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 25050)
    {
      nSubsidy = 45 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 30624)
    {
      nSubsidy = 65 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 36198)
    {
      nSubsidy = 30 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 41772)
    {
      nSubsidy = 45 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 <= 47346)
    {
      nSubsidy = 65 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight+1 > 47347)
    {
      nSubsidy = 30 * COIN;
      return nSubsidy + nFees;
    }
...
const int DAILY_BLOCKCOUNT =  4800;
const int YEARLY_BLOCKCOUNT =  1752000;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 <= 2*YEARLY_BLOCKCOUNT)
    {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Rebel", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Sat, 05 Mar 2016 16:45:40 GMT";
        CTransaction txNew;
        txNew.nTime = 1457196340;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1457196340;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1412781;
		if(fTestNet)
        {
            block.nNonce   = 1412781;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x75b8b8019e0dd9eee4a4553a776cfb380edfa4c63955fefee49a79c8e1067e47"));
        block.print();
